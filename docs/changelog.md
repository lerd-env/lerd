...
- **Optional disabled-DNS install mode for users who do not want lerd touching system DNS**. `lerd install` now asks `Let lerd manage DNS for local sites (No: use *.localhost, no dnsmasq, no HTTPS)?` after the Node prompt and persists the answer under `dns.enabled` in `~/.config/lerd/config.yaml`. When disabled, the installer skips the `lerd-dns` quadlet, the dnsmasq config, the sudoers rule, the mkcert root CA install, and the resolver tweak; `lerd start` no longer manages the lerd-dns unit; `lerd doctor` reports DNS as managed externally; `lerd dns:check` short-circuits to a friendly message; the dashboard hides the per-site HTTPS toggle and the DNS detail panel shows a `disabled` pill; the TUI status bar shows `DNS off` and removes the per-site HTTPS row; the tray shows a muted DNS dot. HTTPS is intentionally coupled, `lerd secure` and the dashboard endpoint return `HTTPS requires lerd-managed DNS, set dns.enabled: true and re-run lerd install` because mkcert never installed a trusted CA in this mode. Re-running `lerd install` with the opposite answer detects the TLD change, lists affected sites, and offers to migrate everything in one pass: stored domains in the registry and `.lerd.yaml`, project `.env` (`APP_URL` plus `VITE_REVERB_*`), git-worktree vhosts and their per-worktree `.env` files, plus stale primary vhost confs and (when disabling) the previous TLS cert and key. The lerd-dns service itself is also torn down on the disable transition (`systemctl stop` + remove quadlet on Linux, `launchctl bootout` + remove plist on macOS). New `docs/features/dns.md` covers the toggle, the `*.localhost` flow, and how to switch back. Suggested by [#281](https://github.com/geodro/lerd/discussions/281).
...
- **Dashboard root page with live widgets and command palette** (#280). Replaces the empty "select a site" landing page with a Dashboard tab built around at-a-glance widgets backed by the existing WebSocket pipeline. The lerd logo at the top of the rail routes here. Eight focused cards in a 3-col grid: HeroStatus alert strip (collapses into the header when healthy), Sites list (registration order, newest first, favicon + framework + worktree count + open-in-browser), Services bounded grid with update banner, Workers per-group active counts plus per-unit detail with the last journal line for failures (via `workerheal.Enrich`), System Health with PHP-FPM and Node.js usage matrices, Resources card (CPU + memory) backed by a new `/api/stats` endpoint that parses `podman stats --no-stream` for `lerd-*` containers, Lerd info merged with a Recent activity timeline driven by a frontend diff over WS snapshots, and a dismissible Onboarding panel when no sites are linked. Plus a global command palette (Cmd+K / Ctrl+K / `/`) for jumping to any site, service, page, or action from any tab.
