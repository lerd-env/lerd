...
## [1.30.0-beta.2] - 2026-07-20

Second beta of the 1.30.0 line. What a PHP version owns keeps shrinking: custom extensions and packages became one declared set applied to every image lerd builds, and php.ini follows with a shared scope that fills the gaps under each version's own file, so a site that changes version stops silently losing what the developer asked for. The dashboard gains a lot of reach this round, managing an engine's databases from its service page with snapshots, export and import, offering to install a service the project asks for but never had, and grouping the services list by type. `lerd doctor` stops merely reporting and learns to repair what it safely can, notifications grow a native desktop sink alongside the new Lerd desktop app, and the terminal dashboard moves onto the Charm v2 stack. Alongside that, `.test` keeps resolving with no network at all on systemd-resolved hosts, and `lerd php` stops leaking a container error when it is run from a directory the PHP container cannot reach.
