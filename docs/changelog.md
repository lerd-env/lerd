...
- **Worktree build prompt and host worker auto-start could disagree on whether to run** (#310 follow-up). `hasHostWorkerAutoStart` ran `MatchesRule` against the parent site path while `syncWorktree` ran the same rule against the worktree path. If a branch removed `vite.config.js` from a Laravel worktree while main still had it, the build prompt was suppressed but auto-start skipped, leaving the worktree without a Vite manifest. The reverse mismatch (worktree has it, main does not) made `npm run build` and the auto-started `npm run dev` race for ports. The suppression check now takes the worktree path so both decisions see the same input.
- **Host workers silently configured a non-functional unit on macOS** (#310 follow-up). `writeWorkerUnitFile_darwin` accepted the new `host bool` param but never branched on it, so a Laravel site on macOS got a launchd plist that ran vite inside the FPM container with no host filesystem access for HMR. The darwin path now early-returns with a `[WARN] worker is host: true which is not yet supported on macOS` so the user knows to run the command manually until the macOS fnm-based path lands.
- **`WorkerStartForSite` variadic `persist ...bool` replaced with named bool** (#310 follow-up). The variadic accepted any number of bools and silently used only the first, so a caller passing two values would get unexpected behaviour without a compile error. The single named bool makes the contract explicit; all eleven existing callers were updated to pass `true` and the worktree auto-start in `syncWorktree` keeps `false`. Pure refactor, no behaviour change.
- **`resolveSiteAndFramework` worktree fallback was untested** (#310 follow-up). The `cwd not registered, fall back to ParentSiteForWorktreeDir` branch added in worker.go had no direct test coverage despite being a behavioural addition. New `TestResolveSiteAndFramework_worktreeFallback` pins it.
...
- **`env_overrides` in `.lerd.yaml` silently suppressed the default `APP_URL` rewrite for new worktrees.** The previous implementation took an all-or-nothing branch: when `env_overrides` had any keys, `EnsureWorktreeEnv` skipped the default `APP_URL=scheme://branch.domain.test` rewrite entirely, even when the user only declared an unrelated key like `SESSION_DOMAIN`. Worktrees ended up pointing at the parent repo's URL. `EnsureWorktreeEnv` now seeds `APP_URL` with the worktree default and layers `env_overrides` on top, so only declared keys override defaults. Implementation moved off a hand-rolled writer onto `envfile.ApplyUpdates`, which gained a skip-write-if-unchanged guard so the move doesn't bump `.env` mtime on no-op scans (vite, IDE indexers, opcache react to mtime). New regression test `TestEnsureWorktreeEnv_partialOverridesStillRewriteAppURL` pins the partial-override case.
- **Dashboard DNS dot still stayed red for up to 30s after `lerd-ui` boot.** The earlier d8820b6 fix added an immediate probe to `WatchDNS` in the watcher process, but the matching watcher inside `lerd-ui` (a separate process with its own `eventbus.Default`) entered its ticker loop without an initial tick, so the dashboard could still sit on stale `dns.ok=false` for up to 30s on boot. `runDNSStatusWatcher` now calls `tickDNSStatus` once before entering the loop, mirroring the watcher-side fix.
- **`cloneProjectConfig` did not deep-copy `env_overrides`.** The project-config cache returns a clone on every read so callers can't corrupt cached state, but the new `EnvOverrides` map was missed in the clone helper, so every reader got the same map reference. Latent today (no in-tree mutator) but fixes the foot-gun before it bites. `TestCloneProjectConfig_DeepCopiesEnvOverrides` regression-pins the isolation.
...
- **MySQL preset wrote two removed-in-8.0 directives that warned on every container start** (#301 follow-up). `~/.local/share/lerd/service-files/mysql/etc_mysql_conf.d_lerd.cnf` shipped `loose-innodb_large_prefix=ON` and `loose-innodb_file_format=Barracuda` for MySQL 5.6 compatibility. Both were removed in MySQL 8.0 and were no-ops on 5.7 (the defaults already match). The `loose-` prefix kept MySQL from refusing to start, but every boot logged `[Warning] [MY-013242] Unknown variable …`. lerd no longer ships 5.6 (only 5.7 / 8.0 / 8.4), so the directives are dropped. A new `TestMySQLPresetExcludesRemovedDirectives` regression-pins the absence so they can't drift back in.
...
- **`<tld>` placeholder rendered as broken HTML in the docs** (#296). Vitepress was parsing the bare `<tld>` literal as an HTML tag and dropping it from the rendered page. Now backtick-wrapped everywhere it appears.
