...
Underneath that, the daemons stopped working while nobody is asking them to. The watcher was rewriting snapshots identical to the ones already on disk ten thousand times a day, re-running a composer install on the same worktree every minute forever because a staleness check could never converge, and sizing its scheduler to a host it has no use for. It now runs at 39 wakeups a second where it ran at 62, for roughly a third of the idle CPU, which is the kind of thing a laptop feels rather than a benchmark reports. The boot scan that used to leave the last worktree unroutable for minutes writes every vhost first and installs afterwards: six worktrees went from seventeen seconds to twenty four milliseconds.

The rest is the long tail of things that had been wrong for a while. A macOS install where a system update quietly cleared the mkcert CA's trust settings looked healthy from every angle while no browser would open a site. A `psql -h 127.0.0.1` against the port lerd itself prints was refused, which is the documented way to reach a lerd database by hand. A worker mounted at `/app` was also swallowing `/appstore`. Orphaned per-worktree units left behind by a coding agent deleting its own checkout restart-looped forever with a heal button that could not work. Six framework walkthroughs, all written from real runs, close the gap between the nine frameworks the store supports and the three the docs knew how to get started with.

A review of everything since 1.31.0 sits behind this release as well, and as usual what it found in this cycle's own new work is described inside the feature it belongs to rather than listed as a fix, since no released version ever had it. What is under Fixed is what a 1.31.0 install actually runs into.

### Added

- **A site is exposed through your own reverse proxy** (#1303). The public tunnels reach the internet through somebody else's service, which is the right trade for showing a colleague a page and the wrong one for a setup where a self-hosted VPN or an nginx in front of the dev box already points a wildcard subdomain at this machine. A public share is the sibling of the LAN share for that case: the same in-process Host-rewriting proxy on a stable port, reached through the proxy the user runs rather than by LAN IP. Set a base domain you control once from the share menu, point its wildcard here, start the share, and lerd answers on the share port while nginx keeps serving the site's normal `.test` vhost, with nothing added to the site's domains, the `server_name` or its certificates. Building it on the LAN proxy means Host rewriting, response-URL rewriting and Vite HMR all work with no new serving code. It is off until started, restored on boot, and cleared when the base is cleared, and worktrees share independently on the flat `<site>-<branch>.<base>` so one wildcard covers them. A site is exposed one way at a time: LAN, public share and tunnel are mutually exclusive, refused in the backend and disabled in the menu while another is live or opening, with a tunnel counting as active from the moment it starts opening rather than only once its URL arrives. A share also carries its reach, because the LAN proxy forwards `/__lerd_vite__/<port>/` to any loopback port on the machine, which is the point on a LAN where a phone has no localhost of the developer's to fetch assets from and is a hole on the open internet, where the same route would hand every visitor the mailpit inbox and the database admin dashboard. A public share closes that route, leaves loopback URLs in response bodies alone rather than rewriting them into a path it refuses to serve, and keeps its own URLs on the https its fronting proxy terminates.
- **Pinggy joins the SSH tunnel tools** (#1301). The SSH mode was shaped exactly like serveo, one hardcoded invocation with the provider host as the only variable, and Pinggy does not fit that form: it listens on 443, wants the server to pick the remote port, and takes a token as the SSH user instead of `nokey`. The mode carries a provider's whole connection shape now, with serveo and localhost.run keeping their exact current invocations, so the next SSH provider is a table entry rather than another special case. Pinggy is opt-in through `--pinggy` or `share:tool pinggy` and auto-detection is unchanged. Tokens became per provider along the way: `share:token` takes an optional provider argument, a bare token still means ngrok, and a token stored with `share:token pinggy` moves the connection to `pro.pinggy.io` for a stable subdomain. The free tier hands out both `*.free.pinggy.net` and `*.run.pinggy-free.link` URLs, confirmed against a live tunnel, and the URL pattern accepts either so the dashboard learns the public URL wherever the share was started.
- **Reusable snippets in the Tinker tab** (#1299). The tab gains a snippets menu modelled on Tinkerwell's project snippets. Reusable `.php` files are read from `.lerd/tinker/snippets` in the project, from `.tinkerwell/snippets` so existing Tinkerwell users keep theirs without moving them, and from `~/.config/lerd/tinker/snippets` for personal snippets that follow the user across every site, with one file per snippet labelled by a `// @name` header or its filename and worktrees reading from their own checkout. Loading a snippet only fills the editor, since running something on selection is not what a menu should do. Snippets are saved and deleted from the same menu, writing to the project or the global folder and never into the tinkerwell directory, with a confirmation on every step that could lose work: replacing a non-empty editor, overwriting an existing file, and deleting. The backend serves it all on a single site action, `GET` lists, `POST` saves and `DELETE` removes, branch aware for worktrees, with names validated down to a bare `.php` filename and content capped at the same 64 KB the tinker run request accepts.
- **Managed services are reachable from the LAN, if you ask for it** (#1247, #1259). A LAN-exposed install served the sites and nothing behind them, so a phone on the WiFi could open the app while a colleague's machine could not reach the database it runs on. Managed engines can now be published to the LAN as a deliberate opt-in, and because that is a decision about the network rather than about a service, the switch lives inside the LAN exposure card under the control it depends on and stays hidden while lerd is bound to loopback, where flipping it would only have stored a preference for later. The CLI and the API refuse to enable it there too, so the dashboard is not the only thing holding the line, while turning it off always works and a setting that is already on keeps showing after an unexpose so re-exposing cannot quietly republish databases. Rebinding converges rather than reporting success optimistically: every affected unit is attempted and the failures are reported together, and which units to restart is decided by comparing each installed container's published ports against the policy it should be following, so a toggle interrupted halfway, which leaves the quadlets rewritten while the containers still run their old bind, is noticed and cleared instead of surviving every later run.
- **Remote dashboard actions that reach the host are behind their own switch** (#1247, #1320). An authenticated remote session drives the dashboard, but the actions touching the machine itself, raw `.env` reads, filesystem browsing, database drops, terminals, tooling replacement and shutting lerd down, stay local unless the user asks for them. The route list is unchanged and the gate consults `ui.remote_full_access` rather than refusing outright, so an install that has not opted in answers a remote client exactly as it did before. The opt-in lives on `lerd remote-control full-access on/off/status` and on a toggle in the Remote dashboard access card, both of which refuse to run from a remote session so a session can never widen its own authority, and `remote-control off` clears the setting along with the credentials instead of leaving it armed for whatever password is set next. Authority is asked for by name now, `hasHostActionAuthority` for the handlers already sitting behind the gate and `hasDashboardControl` for the ones that have to hold up on their own, which leaves `isLoopbackRequest` meaning loopback again rather than doubling as a stand-in for an authenticated session. That distinction is the whole point: a reverse proxy relaying someone else's browser connects from 127.0.0.1, so a loopback request carrying `X-Forwarded-For`, `X-Forwarded-Host`, `X-Real-IP` or `Forwarded` is treated as remote, in the handlers as well as the middleware, and the near-duplicate helper the two had drifted between is gone. The `Host` header is deliberately not part of that decision, since a browser on the machine may legitimately arrive as the hostname or an `/etc/hosts` alias and turning those away leaves the local user with no way in.
- **`lerd worktree wait` blocks until the setup pipeline settles** (#1256). The watcher's auto-setup is what makes a bare `git worktree add` from any tool a first-class flow, and a tool that then wanted to act on the new tree had no way to ask whether that pipeline had finished. The only wait lived inside the interactive wrapper, leaving everyone else to probe the tree's contents or reverse-engineer the install lock, and both are traps: composer's extraction phase fills existing vendor directories and `node_modules` exists from the very first extracted package, so file counts and mtimes sit still through the longest stretch of an install and read as finished. Two installers writing one tree is how vendor ends up holding packages but no `autoload.php`, which presents as a Composer bug rather than as the race it is. The command blocks for a path, defaulting to the current directory, and exits 0 once the tree is safe to touch, 1 on timeout and 3 when the path is not a worktree lerd manages, so a caller can tell "ask someone else" apart from "still going". Readiness consults the install lock alongside the pipeline's outputs, which closes the same false-ready window in `worktree add`'s own wait, through a shared non-blocking flock rather than a try-lock so the probe neither overwrites the pid the real holder recorded nor claims a lock a starting installer would have to poll for. The MCP worktree tool carried the identical race in its add action, which ran bare git and handed the tree straight back to the assistant; it waits by default now and reports whether the tree is provisioned, a `wait` action covers a worktree created outside the tool, an install still running is reported as not-yet rather than as a failure, and the reference injected into assistants states that readiness must never be inferred from the tree's contents.
- **A quality tool analysing a file the container cannot see gets it staged in** (#1321). PhpStorm runs its quality tools against a temp copy of the buffer you are editing rather than against the file in the project tree, and on Linux that copy lands under `/tmp`, which lerd deliberately never bind-mounts, so phpstan started fine and then reported that the path it was told to analyse did not exist. The script-operand rescue could not answer this one, since `/dev/stdin` is not a path a tool can stat, reopen or write back to. Every absolute path argument that exists on the host but is invisible to the container is copied into a scratch directory under the lerd data dir now, with the argument rewritten to point at the copy, and the tool's stdout and stderr stream through a rewriter mapping the scratch path back to the one the caller passed, in both the plain and the `json_encode` escaped shape, so the IDE can key its diagnostics to the file it knows about. A copy the tool edited in place is written home again before the command returns, which is what keeps php-cs-fixer and phpcbf working rather than silently discarding their fixes. Staging stays dormant when no argument needs it, the scratch tree is removed when the command exits, and a path above 64 MB is refused with a message naming the two real ways to reach it, since at that size it is a data file wanting a mount rather than IDE scratch.
- **`lerd path:disable` keeps lerd's shims off the shell PATH** (#1282, #1295). Some users prefer typing `lerd php` explicitly and keeping bare `php` pointed at a host install, for example so a script run through PHP's `exec()` sees the real host environment, and the only way was deleting the rc line by hand, which the next `lerd install` or `node:manage` run wrote straight back. The choice is persisted as `shims.path_disabled` and toggled with `lerd path:disable` and `lerd path:enable`. Disabling removes the PATH entry from every shell location, bash, zsh and fish's `conf.d` file, and every code path that used to write it honours the flag, so installs and updates leave the rc alone, while the shim scripts themselves are still written and lerd keeps injecting its bin dir into the child processes it spawns. Making that last part true took a fix of its own: framework commands are shell strings beginning with `php`, and while the dashboard prepended the shim dir before handing one to a shell, `lerd run` and the MCP `commands_run` did not, so with the shims off the PATH every quick command died with `php: command not found` from the CLI and from an assistant while the same command worked from the dashboard. The prepending is one helper the four host-shell callers share now. Shell completions survive the toggle, and the rest was checked rather than assumed: the TUI shells out to the lerd binary so it inherits whatever the CLI does, setup steps exec inside the container, both worker paths build their own PATH starting at the shim dir, and `lerd console`, `lerd php` and `lerd composer` resolve the container directly.
- **dnsmasq is offered on NetworkManager-only hosts** (#1305, #1302). NetworkManager without systemd-resolved resolves `.test` through NM's dnsmasq plugin, and nothing pulls the dnsmasq binary in on Arch, so a fresh install sailed through the prerequisite check and hit the wall later, when lerd refused to configure NetworkManager and asked for a manual install. The installer detects that combination when managed DNS is selected and queues dnsmasq with the other missing packages, `dnsmasq-base` on the Debian family where the full package drags in a system service that fights NetworkManager over port 53. The same gap on an existing install is a doctor rung rather than a silent failure. The lookup checks the sbin directories as well as PATH, since a normal user's PATH omits `/usr/sbin` on Debian, which is exactly where apt puts the binary while NetworkManager finds it through its compile-time path regardless, and it lives in one shared helper the setup guard and the doctor rung both use so the two can never disagree. Hosts running systemd-resolved are untouched and `.localhost` mode skips the check entirely.

### Changed

- **The resident daemons stop working while the machine is idle** (#1271). The watcher rewrote the request-timing snapshot every ten seconds and the idle activity map every thirty, for the life of the process, whether or not anything had changed, which on a quiet machine is over ten thousand write-and-rename cycles a day to persist bytes identical to the ones already on disk. The cost that matters there is not CPU, it is that the disk and the writeback path never get to settle, which is exactly what a laptop feels. Both writers compare against what is already there and skip the write when it matches. Sampling context switches in two-second buckets then showed the idle cost was not spread across the timers at all but concentrated in one spike every thirty seconds, and the worst of it was a bug rather than a design decision: the staleness check for a project's dependencies compares the install marker's mtime against the lockfile, and a git worktree is seeded with a copy of the main repo's vendor directory, so its marker arrives carrying the main repo's older timestamp against a freshly checked out lockfile and the check could never converge, which had the recovery rescan re-running `composer install` on that worktree every minute, indefinitely. Markers are stamped once the install that owns them has succeeded, which is the only point at which the tree is known to match the lock. The rest went the same way: the host-gateway watch forked podman every tick to read an address that only moves when the container is recreated, the idle engine asked systemd for each suspended site's timer through a call that fetches the unit's entire property dictionary to read one field where the batched snapshot the dashboard keeps warm has it, the source-file watcher re-walked every watched tree on each pass when new directories already arrive as create events on their parent, worker detection re-read and re-decoded a site's `composer.json` once per rule per site, and the tick built a site-domain resolver up front to answer a question that almost never gets asked. The three daemons also sized the Go scheduler to the host, which on a many-core machine gave each of them dozens of threads and turned every wakeup into futex traffic across all of them; they are event handlers waiting on sockets, timers and subprocesses, so `GOMAXPROCS` is capped at four unless the environment says otherwise, and goroutines blocked in syscalls still get threads of their own so subprocess and file concurrency is unchanged. The MCP server gets the same cap, since it is spawned by an assistant rather than by systemd but lives as long as the session and someone working with a couple of editors open ends up with one per session. Together the watcher goes from 62 wakeups a second to 39, at roughly a third of the idle CPU, thread counts across the three roughly halve, and the systemd DBus unit-state watcher is gone, since nothing ever called it.
- **Homebrew on Linux installs from the tap** (#1275). The brews config only fed the darwin archives to the formula, so GoReleaser emitted `depends_on :macos` and Homebrew on Linux refused the install outright. The Linux archives are included now, which makes GoReleaser generate `on_macos` and `on_linux` blocks instead, with the podman dependency scoped to macOS since on Linux lerd integrates with the distribution's podman and its systemd and a brew-installed one would be the wrong one to pull in. The release notes template points Linux users at the apt, dnf and Homebrew options. The 1.31.0 formula was regenerated by hand in this shape and verified on Homebrew for Linux; from this release GoReleaser produces it on its own.
- **Every command run in the PHP container sees the terminal's real colour capability** (#1287). podman forwards none of the host environment, so Symfony Console and laravel/prompts never saw `COLORTERM` and fell back to the `TERM` podman pins to `xterm`, which flattened anything styled with hex or 256 colours while the same code against a host php rendered exactly. The exec paths with a real terminal attached carry `COLORTERM` across now, together with `NO_COLOR` and `FORCE_COLOR` so a choice made on the host holds on both sides of the boundary, covering `lerd php` and everything routed through it, the framework console, `lerd shell` and the shell opened from the TUI. `TERM` itself stays out of it: the images only carry terminfo for the xterm family, and Symfony matches `TERM` against a fixed pattern that foot and wezterm fail, so forwarding the host value would turn colour off for exactly the terminals the pinned xterm keeps working today, while a terminal advertising more than sixteen colours gets `xterm-256color` and the eight-bit path back. `lerd tinker` keeps its deliberate `NO_COLOR` and `TERM=dumb`, since its output is captured and rendered by the UI rather than written to a terminal.
- **npm globals belong to the user rather than to lerd** (#1282). The npm shim forced `npm_config_prefix` to lerd's node-global dir for every npm and npx run, so `npm install -g` landed inside `~/.local/share/lerd`, which made globals invisible the moment the PATH shim was removed and deleted them outright on an uninstall with data removal, which nobody expects from removing a dev tool. A prefix the user configured themselves, `npm_config_prefix` or `NPM_CONFIG_PREFIX` in the environment or a `prefix=` line in `~/.npmrc`, wins now: installs land there, no wrappers are written, and their globals never depend on lerd, with the managed prefix remaining the fallback for users who have none of their own. Globals that did land in the managed prefix are no longer silently lost, either. `node:unmanage` lists them as `name@version` specs, keeps them on disk and removes the now-dead wrappers so a reinstall with the user's own npm is not shadowed, and `uninstall` with data removal offers to reinstall them with the system npm before deleting anything, printing the exact `npm install -g` line when that is declined or no system npm exists.

### Fixed

- **A macOS update could clear the CA's trust and leave every site's HTTPS broken with nothing reporting it** (#1237). `CATrusted()` only checked whether mkcert's root CA was present in the keychain, never whether it actually carried trust settings, and a certificate can sit there with its trust cleared independently of the certificate item itself, which is most commonly what a macOS update does. `security find-certificate` reports that identically to a genuinely trusted CA, so lerd treated a reinstall as a no-op, skipped the sudo announcement, and left every site's certificate invalid in the browser while podman and the vhost looked completely healthy. The check parses `security trust-settings-export -d` now, which is keyed by each certificate's SHA-1 fingerprint, and the presence of an entry is the trust decision, since per `SecTrustSettings` an absent or empty `trustSettings` array means trust as a root and only mkcert writes an explicit one; only an entry whose explicit results all refuse counts as untrusted. mkcert's own "already installed" self-check cannot be relied on to notice this, because it verifies the self-signed root against itself and succeeds cryptographically regardless of trust settings, which is the same false positive, so `ensureMkcertCA` repairs trust directly with the `security add-trusted-cert` command mkcert itself uses before mkcert gets a chance to decide there is nothing to do. An export that could not be read reports nothing rather than untrusted, so a failing `security` cannot drive a privileged repair on its own, and a failed repair is named rather than swallowed since macOS draws the authorization dialog itself and a run with no window server fails there.
- **`psql -h 127.0.0.1` against lerd's own published port was refused** (#1235). Every client-tool shim execs its tool inside a throwaway container joined only to the lerd network, so while a hostless `psql -l` routed correctly to the internal `lerd-<service>` container name, an explicit `-h 127.0.0.1` or `-h localhost` at the port lerd published always failed with connection refused, because inside that throwaway container the loopback address is its own rather than the host's. That is also the documented way to reach a lerd database from the host, so it hit anyone running psql, pg_dump, mysql or mysqldump by hand or from a script against the port `lerd service start` prints. A loopback host whose port matches one of lerd's own installed services is recognised and routed exactly like a hostless call, disambiguating among several installed same-family instances, while a loopback host matching nothing lerd owns or any other explicit host still passes straight through. A `PGHOST` or `MYSQL_HOST` left set in the shell can no longer silently override an explicit `-h` flag's loopback match either, matching how a real client always lets the flag win. A connection URI or a libpq conninfo string keeps its own host even when that host is a loopback spelling, which matters more than it sounds: the host decision had moved to a flag-only test, so a URI read as no host at all, fell into the local-default branch, and arrived at an external server carrying lerd's own admin credentials in `PGPASSWORD`.
- **A worker mounted at `/app` also swallowed `/appstore`** (#1235). The worker proxy location block was a bare `location /app { }`, which nginx treats as a prefix match, so unrelated routes merely starting with the same characters were silently sent to the worker's backend instead of being served by the site. It is anchored as `location ~ ^/app(/|$) { }` now, which makes the path a regex, and since it comes straight from framework store YAML as a literal string it goes through `regexp.QuoteMeta` first, without which a path like `/socket.io` would have its dot read as any character and match `/socketXio` too. The path is normalised before anchoring, because three ways of writing one produced a block matching nothing: a trailing slash rendered `^/app/(/|$)`, which matches the literal `/app/` and nothing else, a bare `/` rendered `^/(/|$)`, which matches `/` and `//` and nothing beneath, and a path written without its leading slash rendered `^app(/|$)`, which can never match a URI that always starts with one. A proxy block declaring no path at all is reported as no proxy rather than quietly capturing every route. The docs note that the anchoring makes nginx evaluate the location in file order ahead of the php and dotfile-deny blocks, so a `.php` file or dotfile under the proxied path is proxied rather than served, which is right for a worker mounted on its own path and worth stating.
- **Orphaned per-worktree units restart-looped forever behind a heal that could not work** (#1245, #1323). A per-worktree unit pins its working directory to the checkout, so deleting that directory leaves systemd retrying a command that fails at CHDIR before it ever runs. Removing a worktree through lerd tears those units down first, but deleting the checkout on its own leaves git's own worktrees entry in place and nothing ever hears about it, which is how an install ends up with several of these thousands of restarts deep: coding agents create and destroy their own worktrees and have no reason to know lerd exists. Such a unit is told apart from a failure and removed rather than restarted, since starting it again can only fail in the same place, and the distinction cannot rest on what systemd calls it, because a unit that cannot chdir never settles into failed and flaps between activating and active for as long as the machine is up. Reconciliation runs on the daemon's own cadence and again at startup, so an install already in this state recovers without anyone having to learn there was something to fix, and orphans stay out of the failing-worker notifications since they name a problem that resolves itself. macOS reaches the same symptom and was missed at first, because launchd exposes no working directory to read back and the function supplying it returned nothing, so every unit resolved with an empty probe path; it is read out of the guard script lerd writes for every worker instead, where a host-mode worker's `cd` and an exec-mode worker's `podman exec -w` each record it, which needs no migration and no change to any plist. A unit is only deleted when the checkout is genuinely missing, not whenever `stat` fails, so an unmounted disk or a permission error on a parent no longer costs a worktree its workers.
- **The watcher could be killed by systemd before it ever came up** (#1261). It runs under a `Type=notify` unit and signalled readiness only after its boot work had finished, and that work provisions every worktree it finds needing it, composer install included, so an install slower than the unit's start timeout earned a SIGTERM before readiness was ever signalled. systemd recorded a start timeout, restarted the unit, and the fresh process began the same install from the beginning to be killed at the same point, restart counter climbing and nothing converging. What the user sees is a watcher that is simply down, so nothing provisions worktrees and nothing reacts to new projects, and recovering by hand means knowing to stop the unit, run the install outside it and start it again. Readiness goes first now and the boot scan runs in the background the way the later periodic passes already do, since none of that scan is a precondition for serving. Raising the start timeout was the other option and it only moves the cliff, because an install has no duration worth guessing at.
- **The last worktree on a repo stayed unroutable for minutes after boot** (#1281). The boot scan walked a site's worktrees one at a time and, for each, ran the dependency install before writing the vhost, so a subdomain waited not only on its own composer and npm run but on every worktree ahead of it in the list. What that looks like is a worktree that was added successfully, has its vendor and its env in place, and 404s, with nothing in status or doctor pointing anywhere. Nothing about routing depends on the install, so the scan writes every vhost and reloads nginx first and hands the rest of each worktree's reconcile back as a job to run afterwards, several at a time since one worktree's install has nothing to say to its neighbour's. Six worktrees on a four-core machine went from the last vhost landing seventeen seconds in to all six landing in twenty-four milliseconds. Two cores stay out of it, because this runs unattended at daemon start next to the containers serving everything else. The vhost that scan wrote also named the parent site's PHP container rather than the version the worktree pins in its own `.lerd.yaml`, which every other place that materialises a worktree vhost gets right, so a branch held back on an older PHP was served by the newer one from the first pass after any watcher restart. A lockfile the JS package manager refuses is the same lockfile in every worktree seeded from that repo and fails the same way in each, several seconds at a time, so that failure is paid once per pass now and the remaining worktrees carrying it are logged as skipped, keyed on the lockfile's contents so a branch carrying a different one still gets its own attempt and cleared between passes so a fixed lockfile is tried again.
- **The dashboard opened with no sites at all after `lerd-ui` restarted** (#1260). A caller that lost the snapshot build lock was handed whatever the cache already held, which on a cold cache is nil, and the handler wrote that out as a zero-byte body, so a browser opening the dashboard just after a restart lost the race on most of the requests it fires at once: twenty-five concurrent reads of `/api/sites` returned an empty body to twenty-four of them. On the client that empty body fails to parse and the catch keeps the value the store was initialised with, so the dashboard showed nothing and stayed that way until some unrelated mutation published a sites payload over the websocket, which on an idle machine can be minutes. A caller holding a usable stale value still returns immediately rather than queuing behind a slow `podman inspect`, which is why the early return existed, but a caller with nothing to show waits for the build in flight, because answering with the cold cache is answering with a claim about the registry that nobody checked. From the other direction, the builder turned a failure to read the registry into an empty slice and the cache stamped it fresh, serving one transient failure as the truth for up to five minutes; it reports the error instead, a failed build is never cached, and the handler answers 503 rather than an empty list, while a failed rebuild over something already cached keeps serving it and leaves the timestamp alone so the next read tries again. Worker health still reports an empty list when its probe fails, since no unhealthy workers is a safe reading of that, unlike no sites. Nothing re-fetches the sites list on a timer, so the client retries while it has never managed to load and leaves a list that is already on screen alone.
- **The browser HTTPS check reported a pass whenever certutil was installed** (#1258). It answered a question about the tool rather than about the store it is named for, so a machine where nss-tools is present but the root CA never reached the NSS store, or was removed from a profile afterwards, got a clean bill of health while every browser refused the certificates lerd signs, which is the one symptom the check exists to catch. It looks for the CA in the databases mkcert installs into now, the store the Chromium family shares and every Firefox profile carrying one, and compares what it exports against the CA lerd signs with rather than accepting any certificate wearing the mkcert name, so a CA regenerated after a profile imported the old one is reported instead of passing. A store that comes up short is named in the warning and repaired by `lerd dns:repair`, missing certutil keeps the guidance it already had, and nothing to ask with, no CA yet, or no browser store on the machine all report nothing rather than a warning, matching how the system trust checks already treat a question they could not put.
- **A postgres service whose data outlived its quadlet came back on the wrong major** (#1254). A default preset backfilled its canonical version pin from the image named in whatever quadlet it found on disk, so the version was only ever recovered from an install that still had one, and when a data dir outlived both its quadlet and its config entry the probe came back empty and resolution fell through to whatever the bundled YAML currently calls canonical, which mounts postgres 16 over a cluster written by 18 and leaves a container that refuses to start. Reinstalling made it worse rather than better, since the captured pre-remove tag came from that same stale quadlet. A preset declares where its engine records the version that wrote its data now, `PG_VERSION` for postgres, and that reading wins over every pin derived from the quadlet, the config entry or a reinstall, so the strategy, `track_latest` and reinstall branches all end up on a server that can actually open the files. An explicit image in `config.yaml` stays authoritative, because that is a deliberate choice and silently rewriting it would be worse than the failure it papers over. Engines with no readable marker declare nothing and resolve exactly as before, mysql among them, since 8.4 keeps the version in its data dictionary rather than a text file. The read goes through the user namespace, because a service data dir is owned by the container's mapped uid with mode 0700 and a plain read would fail in a way indistinguishable from an absent marker.
- **A dev server kept advertising the scheme and domains the site no longer had** (#1241). It reads the origin and the allowed hosts lerd generates for it once, when it starts, and nothing revisited them, so unsecuring a site left the server still advertising https and every asset URL on the page named a scheme the site no longer serves, arriving unstyled, which is the symptom the base prefix exists to remove. Securing a running site broke the same way in reverse with the plain URLs refused as mixed content, and moving a domain surfaced as the server rejecting the host outright. The generated config and the server running on it follow from the two places every caller already shares, the HTTPS toggle and the vhost regeneration every domain change goes through, so the CLI, the UI, MCP and the group cascade all reach it without any of them naming it. The decision rests on the contents of the config rather than on the event, so the many paths that regenerate a vhost without moving an address never bounce a dev server somebody is working against, and one that is not running is left down. A site with several domains serves all of them from one origin, which is inherent since a dev server can advertise only one; what was not inherent is that naming an origin at all makes some framework plugins adopt it as their entire CORS allowlist in place of a default that accepted any domain of the site, so a page opened on a second domain had every module script refused, and the generated config declares those origins itself. The allowed hosts mirror the vhost as well, each domain and that domain's wildcard, which is how a worktree subdomain reaches the dev server.
- **A dev server told a plugin its address was `http://[::]:5173`** (#1250). The generated config names `server.origin` and leaves it to the framework plugin to publish assets from there, and a plugin released before Vite grew that option ignores it and publishes whatever address the server bound to, which with `host: true` is the IPv6 wildcard. Nothing can route to that address, and on a secured site the browser refuses the plain HTTP request as mixed content, so the page arrives unstyled. The config carries the one URL a browser reaches the server at, the site's origin joined to the prefix the vhost proxies, and watches for that address on its way to disk during startup: a bare host and port on the port this server is listening on is replaced with the public URL, which is what a current plugin writes there anyway, and anything else passes through untouched.
- **A dev server made its site look like the busiest one on the machine** (#1243). It answers on the site's own domain under the prefix lerd gives it, so everything it served arrived in the request-timing feed looking like a route the app had served, and the static-asset test never stood a chance, since the modules a dev server rewrites carry extensions the asset list has no reason to know and the ones rebuilt on every save ended up reading as the busiest routes on the site. The exclusion sits with the two that were already there, so every reader and writer of the timing data still agrees on what counts. The site usage query had drifted from that agreement, testing only the extension while its comment claimed it shared the predicate, so a dev server made a site look busy in the sites list as well; both decide through one test of the URI now, and the dev server tools keep their own prefixes rather than reqstats restating them.
- **npm global wrappers lost the prefix as soon as they activated Node** (#1269). The wrappers lerd generates activate Node through fnm or nvm, which clobbers `npm_config_prefix` with the version manager's default, so tools like `codex doctor` reported updates targeting a different installation than where the global actually lives. The wrappers inject the prefix after activation now, fnm through an `env KEY=VAL` appended after `--` and nvm through an `export KEY=VAL;` embedded after `nvm use`, since nvm aborts when the prefix is set before activation. The values are shell-quoted, which the nvm side already did and the fnm side did not: the prefix is interpolated verbatim into a `/bin/sh` wrapper, so a data directory containing a space split into two words and `env` took the tail as the command to run, which fails before ever reaching the real binary and is a worse outcome than the missing prefix it was added to fix.
- **VSCode rejected every lerd MCP tool** (#1267). Every tool property typed as an array was emitted without the `items` field the MCP spec requires to describe the element type. Lenient clients ignored it, VSCode strict-validates and refused every tool with "tool parameters array type must have items". Each array property declares its element type now, and the two that are not string arrays say so: the framework handler parses `setup` as label, command, default and an optional check, and `logs` as path with an optional format, both objects. Declaring string elements there would have been worse than declaring none, because a strict client, which is the entire reason for adding `items`, then sends strings, the map assertion in the handler skips every element without raising anything, the call still reports the framework as saved, and the file comes back with no setup and no logs in it.
- **`lerd update` failed with "unexpected release URL format"** (#1298). The update checker resolved the latest version by parsing the tag out of the first `Location` header returned by `/releases/latest`, and a repo or org rename inserts an extra hop, where GitHub answers the old path with a redirect to the new `/releases/latest` URL and only that second URL redirects to the tag. Every pre-rename binary therefore failed. The checker follows the chain up to five hops until a `/tag/` URL appears, while a malformed tag URL still fails immediately, and the troubleshooting page gains an entry for the error with the one-time reinstall that pre-1.26 installs need, since on those builds the updater itself is the broken part and no release can reach them.
- **A snapshot whose dump failed was stored as restorable** (#1336). The dump was piped into gzip, which succeeds regardless of what happened upstream, so a failed dump was written as a successful snapshot holding an empty gzip stream, twenty bytes that `meta.json` recorded as restorable. Restoring it would have replaced a live database with nothing, and because restore drops and recreates before it reads the dump, that is exactly what it would have done. The dump runs with `pipefail` so a dead dump's status survives the pipe, and the archive is read back and rejected when it decompresses to nothing, which catches the same failure on shells that do not know the option. Verifying only at creation would have left every snapshot written before this on disk and still listed as restorable, so the guard is on the restore side too and runs before the drop.
- **A site's schema missing from a running engine returned 500 with a clean doctor** (#1336). The missing-database check only ever covered SQLite, so a site whose MySQL or Postgres schema does not exist failed every request while the doctor reported nothing wrong. It checks a server schema too now, through the list command the service preset declares, and stays quiet when the engine cannot be queried so a stopped service never reads as a missing database.
- **`lerd start` reported the stack ready while the engines were still booting** (#1336). systemd calls a unit active as soon as the container is up, which for a database is well before it accepts connections, so the first request to a database-backed site returned 500 until it caught up. Start waits for the engines it started to accept connections now, bounded so a wedged one cannot hold the whole start hostage, and it does not wait at all for a DNS container that external DNS mode deliberately does not run. Switching a site's PHP version had the same shape: it repointed the vhost at a backend whose container was not running and returned, so the first request got a 502. The switch starts the unit when it is down and waits for php-fpm to answer on the port nginx proxies to, probed through PHP itself since that is the one interpreter an FPM image is guaranteed to carry, bounded so a missing or broken image cannot hold the command open and costing nothing when the container is already up. `secure` and `unsecure` both printed the new URL and returned while the previous vhost was still answering, because an nginx reload is asynchronous, so opening the URL the command just printed got a redirect to https from the stale secured vhost or no connection at all on an https endpoint that was not live yet; both wait for nginx to serve the new scheme, a secured site counting as served once its TLS endpoint answers at all since waiting for a 200 would sit out a site that is simply broken.
- **Securing a Symfony site kept generating http links** (#1336). Securing wrote `APP_URL` into `.env` whatever the framework said, so a Symfony site served over https kept its `DEFAULT_URI` in `.env.local` at the old scheme with nothing ever updating it. The env file and the URL key come from the framework definition now, through one pair of helpers every path that writes a site's URL goes through, so no caller assumes Laravel and a framework declaring no URL key has nothing written for it.
- **Linking a directory twice registered it as two sites** (#1336). A directory already registered under a different name was added a second time, with duplicate vhosts and duplicate workers, and `unlink` refused a site name at all, so neither registration could be removed without cd-ing into the directory. Linking refuses and names the existing site now, and unlink takes the name the reference always documented. The guard behind that compared canonical path strings, and resolving symlinks only folds together the spellings that pass through a link, so a case-insensitive volume, which is what macOS formats by default, spells one directory many ways and kept every one of them through `EvalSymlinks`. Identity is asked of the filesystem instead: both paths are stat'd and compared with `os.SameFile`, which settles case, symlinks and bind mounts together and needs no knowledge of the volume, falling back to the canonical strings when either path cannot be stat'd so a registry entry whose directory has since been deleted still matches itself. Every place that asks whether two paths are the same directory goes through it, because the name axis alone was not enough: the guard held while the domain axis still disambiguated, so re-linking one project through another spelling of its own path kept its name and moved it to `app-2.test`.
- **A FrankenPHP switch that could not come up reported success** (#1336). It built the image and started the unit but treated a failed build and a failed start as warnings, and with no image the quadlet names `localhost/lerd-frankenphp85:local`, which podman reads as a registry and fails a TLS handshake against, so the unit restart-loops while the site sits on 502 with nothing explaining why. The switch refuses to continue when no usable image is present, tolerating a failed rebuild only when one already is, and checks that the container actually settled into running rather than trusting the start call, which returns before a quadlet unit has resolved. The site's recorded runtime is written before the container exists, so a switch that cannot complete restores the previous runtime and restarts the workers it stopped rather than leaving the registry claiming a runtime that is not serving.
- **The mysql preset's database actions depended on a matching socket path** (#1336). It addressed its client without a host on every database action, which left the commands relying on the client's compiled-in socket path matching the one the server listens on, and where those differ create, export, import and snapshot all failed. The preset names the host the way the migrate path in Go already did.
- **`install.sh` skipped `lerd install` and still reported success** (#1336). It guarded its `/dev/tty` redirects with a readability test, which passes on the device node even when the process has no controlling terminal, so the redirect failed and the install step was skipped while the script reported success, leaving a fresh host unconfigured with no visible error. Detection opens the device now, and `ask` seeds its answer variable, because `set -u` turned a read that never ran into an abort rather than a declined question.
- **`dns:check` warned on hosts where NetworkManager runs its own dnsmasq** (#1336). It probed systemd-resolved on a host where resolved is masked and NetworkManager owns DNS through its dnsmasq plugin, turning a healthy install into a warning that read like a raw command failure.
- **Removing a worktree reported a failure nobody could diagnose** (#1336). The error was a fixed string that discarded git's exit status and its stderr, and the argument reached git unchanged, which meant the branch name `worktree add` takes was rejected as not a working tree. git's own message travels with the error now, and a branch name resolves to the checkout it created.

### Docs

- Six framework walkthroughs are added, written from real runs rather than from the definitions: Drupal (#1289), CakePHP (#1290), CodeIgniter, Statamic, Tempest and Magento (#1293), closing the gap between the nine frameworks the store supports and the three the docs knew how to start. The Drupal and CakePHP pages follow the definition fixes behind them (#1294).
- Site Management is split into focused pages and the menus regrouped (#1308). It had grown to 550 lines answering six different questions from one sidebar entry, so domains, workspaces, pausing and public tunnels each get their own page under usage, request timing moves to features beside the Profiler and Queries pages it works with, the old `/usage/sites` URL stays live and every inbound anchor is repointed. Framework Commands moves to usage so opening it no longer swaps the sidebar over to the features section, and the features menu is regrouped to mirror the features index, picking up the Logs page that was never listed anywhere.
- The packaged install routes are documented end to end (#1274, #1308): which Ubuntu releases the PPA publishes for and how to recover from a source entry left behind on one it does not, the Fedora COPR alongside apt, Homebrew on Linux, and what an uninstall does with a package-owned binary.
- Pointing a project at a host-installed database or service instead of lerd's container gets a recipe on the Services page (#1288), with the Linux prerequisites spelled out, since the claim that a containerized app reaches a host MySQL over `host.containers.internal` with no extra setup holds on macOS and not on Linux, where the connection arrives from a real non-loopback address that Ubuntu's default `bind-address` and localhost-only grants refuse.
- A Laravel Herd for Linux landing page is added (#1239), and the README feature list is grouped into sections so twenty-nine dense bullets in one flat run read as something other than a wall of text (#1238).
