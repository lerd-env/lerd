...
## [1.30.0-beta.1] - 2026-07-19

First beta of the 1.30.0 line. Custom PHP extensions and packages stop belonging to a version and become one declared set applied to every image lerd builds, so a site that changes its PHP version keeps what the developer asked for, and lerd records what each version's image actually loaded rather than promising the whole set on a tier that cannot carry it. Alongside that, `.test` keeps resolving with no network at all on systemd-resolved hosts, and `lerd php` stops leaking a container error when it is run from a directory the PHP container cannot reach.
