...
- **Dashboard now ships in German (`de`), Indonesian (`id`), and Dutch (`nl`).** The three biggest PHP-developer audiences without a native dashboard UI get a full set of translated strings (all ~451 keys), wired into the inlang project, the language switcher, and `LOCALE_LABELS` / `LOCALE_CODES`. A vitest check (`src/lib/locales.test.ts`) guards every locale file against key drift from `en.json`. Native-speaker review/polish welcome, especially for `id`.
