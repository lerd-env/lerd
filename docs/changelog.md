...
- **A published tool pin could name any host** (#1203). The manifest that pins composer, fnm and mkcert is fetched from a branch before each download rather than shipped in a release, so it reaches every install without going through one, and a pin was honoured as long as its URL began with https. A pin is now taken only when it points at a host lerd fetches tools from, may carry a sha256 per asset that the download is checked against, and lands in a temporary file that is moved into place only once complete, so a rejected or truncated download cannot replace a working binary.
- **A sudoers drop-in was written for an unvalidated name** (#1203). `lerd bootstrap --system` wrote `/etc/sudoers.d/lerd` for whichever user it resolved, and that name became the first field of every rule in the file with nothing checking it, where a space is enough to produce a drop-in sudo rejects and lose the DNS grant it was meant to install. The name is now checked and the rendered file is put through visudo before it is installed.
...
- **Composer's version read as unknown, and no update was ever offered for it** (#1203). Composer is a phar and lerd has no host PHP to run it with, so its version came only from the stamp written when lerd downloads it, and lerd downloads it only when it is absent. Every install that already had composer showed a question mark on the tools card, and because the update check needs a known version, a newer pinned composer went unnoticed indefinitely. The version is now read out of the phar rather than by running it, and stamped so the read happens once. This affected the first beta as well as every earlier install.
- **An unreachable registry was asked again on every status rebuild** (#1203). The base-image freshness warm-up invalidated the cached answer before reading it, so the failure the first read recorded never held and an offline machine polled continuously. It now reads through the cache, and only the manual check bypasses it.
- **A database import ignored the client image its engine declares** (#1203). Export already honoured it, so an engine whose own image ships no tooling would export and then fail to import.
- **A pinned dependency host was rewritten as a bare substring** (#1203). An environment value naming a versioned install of the same engine came out pointing at a host that does not exist. The rewrite now stops at a name boundary.
- **The JetBrains sync wrote both files even when it changed nothing** (#1203). A sync that left the user's own connection alone still created an empty credentials sidecar in a project it had just decided not to touch, and moved the other file's mtime for nothing.
- **The service stop cascade did not return** (#1203). Dependents are matched by family and env_role rather than by name alone, so a service whose own family satisfies one of its `depends_on` entries is its own dependent and the cascade recursed without end.
