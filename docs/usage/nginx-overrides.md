...
Note that lerd's shipped `client_max_body_size` is `0`, meaning unlimited, so setting a value of your own can only make uploads stricter. If large uploads are failing without an override in place, the limit is coming from PHP (`upload_max_filesize` and `post_max_size` in `lerd php:ini <version>`), not from nginx.
