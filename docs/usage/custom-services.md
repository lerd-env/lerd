...
A third field covers the upstream that cannot be told a mount path at all. Solr serves its UI only under `/solr` and the Mercure hub's only under `/.well-known`, and neither has a setting for it, so `dashboard_proxy_strip` has the proxy remove its own prefix before forwarding instead. The dashboard URL's path carries what is left, which works wherever that UI's links are relative, and redirects naming the upstream's base path are rewritten back inside the mount. A lerd that predates the field ignores it, and the dashboard stays where it was.

Stripping alone is not enough for a page whose own links start at the origin root, since on the shared lerd-ui origin that root is lerd's rather than the upstream's. While the prefix is stripped, `href` and `src` values beginning with a single slash are rewritten onto the mount on their way through, and `dashboard_proxy_rebase` names any further attribute that carries a path the same way, for a page that hands its own router a base path rather than a link. Mailpit's `data-webroot` is one, which is how the mail view is embedded without telling Mailpit to move, leaving `localhost:8025` answering as it always has.
:::
