...
`lerd service reinstall <name>` stops, removes, and reinstalls at the current version, then recreates per-site state on the container that comes back (databases for mysql/mariadb/postgres, buckets for rustfs), skipping anything that is already there. `--reset-data` additionally snapshots every database on the service and wipes the data first. See [custom services](custom-services.md#reinstalling-a-service) for the resolution rules, and [snapshots before a data wipe](database.md#snapshots-before-a-data-wipe) for what to restore from afterwards.
