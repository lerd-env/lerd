...
The Mailpit, Meilisearch and RustFS views are served through the dashboard's own origin rather than framed from their ports, which is what lets them wear lerd's theme: it follows the light and dark you picked, on the surfaces of the palette you picked, and Mailpit's header takes the raised surface instead of Bootstrap's brand blue. Meilisearch's mini-dashboard has no dark mode of its own and paints from a fixed palette, so its colours are mapped onto the theme's as they are read: its page lands on the dashboard's page colour, its panels on the card colour and its pink on your accent. The RustFS console has a light and a dark of its own, so it is handed the mode to open in and its surfaces take the theme's colours. Their own addresses keep working exactly as before, so `http://localhost:8025`, `http://localhost:7700` and `http://localhost:9001` and anything pointing at them are untouched.
