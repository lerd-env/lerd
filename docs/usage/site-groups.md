...
A site group is about **domains**: it rewrites vhosts and reissues certificates so secondaries answer on subdomains of the main. A [workspace](workspaces.md) is about **display only**: it groups sites in the sidebar, the overview and the TUI, and changes nothing about how they are served. A site can be in a group and a workspace at once, and a secondary always shows in its main's workspace.
:::
