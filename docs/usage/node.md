...
On macOS there is a second reason to prefer nvm. The system asks before a tool reads `~/Documents`, `~/Desktop` or `~/Downloads`, and it remembers the answer against the signing identity of the binary that asked. fnm is published without a Developer ID, so its identity changes with every release and a project kept in one of those folders is asked for again after each version bump. nvm is a shell function inside a terminal you have already granted, so it never asks. Moving the project somewhere else works too, and `lerd doctor` points this out when it applies. [Troubleshooting](../troubleshooting.md) has the detail, under the entry about macOS asking for folder access.
...
- **Answer yes**: lerd sets up the bundled fnm, picks the current LTS, and sets it as the default, writing the `node` / `npm` / `npx` shims into `~/.local/share/lerd/bin/`. Per-project version pinning works as described above (`.node-version` / `.nvmrc`). Switch it to your own nvm afterwards with `lerd node:manager nvm`.
