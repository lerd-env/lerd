...
This needs no configuration and no framework definition. A host worker qualifies when the project has the tool installed and the worker command starts it directly, following one level of `npm run` indirection. [Vite+](https://viteplus.dev/) counts too: its `vp dev` starts Vite and forwards lerd's flags to it, so a project whose dev script is `vp dev` gets the same treatment as one that runs `vite`. A command that only reaches the tool through a runner such as `concurrently` is left alone, since the flags lerd appends would land on the wrong process.
