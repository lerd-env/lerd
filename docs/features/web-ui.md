...
- **Site header**: the site's favicon and domain shown as a title, with the project **path** on the line beneath it. Clicking the path opens the folder in your file manager (loopback only). A leading **lock icon** toggles TLS in one click (green closed when enabled, gray open when disabled, static on worktrees and when DNS is off); clicking the domain opens the Manage Domains modal. To the right is an action toolbar: **open in browser**, **group**, **edit nginx** (sliders), a **wifi** toggle for LAN sharing, **xdebug**, **terminal** (loopback only), and a **⋮** menu holding restart, pin, pause/resume, and unlink. The framework badge and a paused indicator sit right-aligned above the toolbar (in the worktree tab strip on a git-backed site). Hovering any icon button reveals a themed label tooltip. When LAN sharing is on, the shareable URL appears as a teal chip with a hover-QR beneath the path. On narrow panels the header stacks into two rows, the identity above and the full toolbar below.
- **Overview layout**: the Overview tab is a stack of sections rather than one long strip, a **Runtime & workers** row with the PHP/Node pickers and worker toggles, a **Services** grid of icon cards, and an **Actions** grid holding the Doctor card and one card per custom command. Live logs moved out to their own **Logs** tab.
