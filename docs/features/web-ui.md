...
- **Services**: the Overview's Services section lists every service from the project's `.lerd.yaml` as an icon card (a category-tinted service icon, its name, and a running/stopped status). Click a card to jump to that service's detail panel on the Services tab. A running service that ships a dashboard (mailpit, an admin UI, and so on) also carries an open-dashboard button on its card that opens the dashboard overlay directly. The section is hidden entirely when the project declares no services.
