...
Run without a terminal, from a script or an assistant, the wrapper asks nothing and takes each default instead: the asset worker or build script the prompt would have preselected, the parent's database unless the framework definition requires an isolated one, and the definition's worktree setup commands. When the definition declares its `worktree.migrations` folder, the database is picked by comparing the worktree's migrations with the parent checkout's: the same set shares, extra ones on the branch get a copy of the parent's database, and a branch missing some of the parent's gets an empty one. The definition's `doctor.migrate_command` then runs on any new database, and on an empty one in every case. The MCP `worktree` tool's `add` finishes the same way, and its `build` and `db` arguments pick a different option.
...
2. Copy main's SQLite database into the worktree when the project runs on one, together with its `-wal` file when present. The file is gitignored, so git never brings it along, and without it the site fails its first request. The path comes from the framework's declared env keys, the same way `lerd doctor` finds it; an absolute or in-memory path is left alone, and a worktree that already has its file keeps it.
...
The same toggle is available on the dashboard for already-active worktrees: flipping **Isolated DB** off opens a confirmation modal naming the database that is about to go, and on confirm it drops the database and restores the parent's database-name value in the worktree's env file. The toggle only appears when the parent uses a lerd-managed mysql/mariadb/postgres service. A SQLite project needs no opt-in: every worktree starts with its own copy of main's database file (step 2 of the [auto-setup pipeline](#auto-setup-pipeline-any-git-worktree-add)), so `lerd worktree add` skips the database prompt and the dashboard's Add worktree form offers only that copy.
