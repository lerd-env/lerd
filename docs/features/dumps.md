...
`ctx.test` is set to `true` on every event captured inside a PHPUnit or Pest run, and omitted otherwise. The signal is PHPUnit's own `PHPUNIT_COMPOSER_INSTALL` bootstrap constant, so it holds for any framework and any test runner built on PHPUnit, and it is a separate field rather than a third `ctx.type` because a feature test that simulates a request is still meaningfully a web or CLI context. A suite fires hundreds of requests and would push every genuine dump out of the buffer, so lerd **does not record tagged events by default**: the receiver drops them before they reach the buffer. Dumping inside a failing test is still one step away: tick **Show test runs** in any lens toolbar and rerun the test. Unticking it removes the test events already buffered, and the setting is saved in the global config so it holds across restarts.
