...
`ctx.command` is the CLI counterpart of `ctx.request`: the invocation an event came from, the script basename plus its arguments (`artisan queue:work --queue=high`), stamped on every cli-SAPI process and capped at 120 characters so a long argument list can't bloat every event. A web event carries `request` and no `command`, a console event the reverse, so anything reporting where an event happened has a name to use whichever side it came from. `ctx.worker`, when the opted-in worker capture sets it, is the more specific label and still wins.
