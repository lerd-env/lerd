...
// assembleSnapshot builds a JSON object containing only the fields whose
...
func assembleSnapshot(sites, services, status, unhealthy, dumpsStatus, devtoolsStatus, profilerStatus, notification, theme []byte, kinds []string) []byte {
	...
	if len(profilerStatus) > 0 {
		buf.WriteString(`,"profiler_status":`)
		buf.Write(profilerStatus)
	}
