<script lang="ts">
  ...
  import { restoreSiteEnv } from '$stores/sites';
  import { diffLines } from '$lib/diff';
  import { m } from '../paraglide/messages.js';

  const target = $derived($modal.envRestore);
  ...
  {#snippet footer()}
    ...
    {#if target}
      <DetailButton tone="primary" onclick={accept} loading={busy} disabled={busy}>
        {busy ? m.envEditor_restoring() : m.envEditor_restoreAccept()}
      </DetailButton>
    {/if}
  {/snippet}
</Modal>
