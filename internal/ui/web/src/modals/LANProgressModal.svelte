      ...
      {#each $lan.progressSteps as line, i (i)}
        <li class="flex items-start gap-1.5">
          {#if i < $lan.progressSteps.length - 1 || done}
            <svg class="w-3 h-3 shrink-0 {error && i === $lan.progressSteps.length - 1 ? 'text-red-500' : 'text-emerald-500'} mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {:else}
            <svg class="w-3 h-3 shrink-0 animate-spin text-gray-400 mt-0.5" fill="none" viewBox="0 0 24 24">
