<script lang="ts">
  ...
  import { restoreSiteNginx } from '$stores/sites';
  import { diffLines } from '$lib/diff';
  import { m } from '../paraglide/messages.js';

  const target = $derived($modal.nginxRestore);
  ...
  async function accept() {
    if (!target) return;
    // Snapshot onSuccess before the await so a concurrent modal store
    // mutation cannot drop the post-restore refresh on the floor.
    ...
    try {
      ...
      closeModal();
      // onSuccess can be async and reject (refresh fetches); catch the
      // rejection here so it never becomes an unhandled promise — the
      // modal is closed by this point so there's nowhere to surface it,
      // but tab-level state shows backupsError/actionError if a refresh
      // fails.
      try {
        await onSuccess?.();
      } catch {
...
<Modal open title={m.nginxEditor_restoreTitle()} onclose={safeClose} size="lg">
  <div class="px-5 py-4 space-y-3">
    {#if !target}
      <p class="text-sm text-gray-500 dark:text-gray-400">{m.common_loading()}</p>
    {:else}
      <p class="text-sm text-gray-700 dark:text-gray-300">
        {m.nginxEditor_restoreBody({ domain: target.domain, name: target.backupName })}
      </p>
