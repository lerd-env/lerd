<script lang="ts">
  ...
  const badgeClass = 'mt-0.5 mb-1 px-1.5 py-px rounded-full text-[10px] font-semibold uppercase tracking-wide';
...
{#if build.channel === 'release'}
  <span class="text-[10px] text-gray-400 dark:text-gray-600 font-mono pb-1">v{$version.current}</span>
{:else}
  {#if build.base}
    <span class="text-[10px] text-gray-400 dark:text-gray-600 font-mono">v{build.base}</span>
  {/if}
  {#if build.commit}
    ...
    <button
      type="button"
      onclick={() => copy(commit)}
      use:tooltip={{ label: copied ? m.common_copied() : hint, placement: 'right' }}
      aria-label={hint}
