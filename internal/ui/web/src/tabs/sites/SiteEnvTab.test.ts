...
const loadSiteEnv = vi.fn(
  async (_d: string, _b: string, file: string) => `FROM=${file}\n`,
);
...
vi.mock("$stores/sites", () => ({
  ...
  loadSiteEnv: (...a: unknown[]) =>
    loadSiteEnv(...(a as [string, string, string])),
...
describe("SiteEnvTab", () => {
  ...
  it("shows the worktree path when a worktree branch is active", async () => {
    ...
    const { getByText } = render(SiteEnvTab, {
      props: { site: wtSite, branch: "feat" },
    });
