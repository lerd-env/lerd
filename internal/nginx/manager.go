...
// forwardedConf declares $real_forwarded_host / $real_forwarded_proto at
// http{} level. Falls back to $host / $scheme when no X-Forwarded-* header
// is present, so `lerd share` and direct browser access both work.
