...
const serverInstructions = "This project runs on lerd. Prefer lerd's tools over the raw commands they wrap: create and remove git worktrees with the `worktree` tool, never `git worktree add` or `git worktree remove`, because add also installs dependencies, builds assets, issues the branch domain and cert and sets up the database. For a new branch off another ref pass branch plus base, never both in git_args. Leave db unset unless the user chose one: add compares the new branch's migrations with the parent checkout's (the branch the site's own folder has checked out) and picks the database itself, with a `Database:` line in setup_output saying why. If that line is missing, the framework declares no migrations folder, so compare them yourself and never assume: extra migrations on the new branch mean `clone-main` and then run them; migrations only the parent has mean the branch is behind, so suggest merging or use `empty`, which add migrates; the same set means `share`. Run console commands and composer through `exec`, and manage services and workers through `service` and `worker`. Call `site` with action list first to find the site."
