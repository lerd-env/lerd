...
type Context struct {
	...
	// Command is the CLI counterpart of Request: the invocation an event came
	// from ("artisan queue:work --queue=high"), stamped by the bridge for every
	// cli-SAPI process so console events can name where they happened.
