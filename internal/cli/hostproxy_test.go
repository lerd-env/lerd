...
func TestHostProxyApproved_wizardPreApproval(t *testing.T) {
	if hostProxyApproved(false) {
		t.Error("hostProxyApproved(false) = true with no pre-approval")
	}
	hostProxyPreApproved = true
	defer func() { hostProxyPreApproved = false }()
	if !hostProxyApproved(false) {
		t.Error("the wizard's pre-approval must count as approval")
	}
}
