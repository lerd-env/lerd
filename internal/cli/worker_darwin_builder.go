...
func buildDarwinHostWorkerGuardScript(execPrefix, lerdBinDir, sitePath, command, extraBinDirs, pidFile string) string {
	...
	// The recorded pid is the process group leader, which is what the stop
	// path's reap sidecar signals to reach children the leader handed off to.
	pidLine := ""
	if pidFile != "" {
		pidLine = "echo $$ > " + podman.ShellQuote(pidFile) + "\n"
	}
...
%scd '%s' || exit 1
%s%s
`, extraPrefix, escapedBin, logcolor.ShellExports(), escapedPath, pidLine, execLine)
}
