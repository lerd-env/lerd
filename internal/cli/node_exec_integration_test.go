...
func TestRunNode_FailingScriptReturnsErrorNotExit(t *testing.T) {
	...
	// Without a default Node, runNode returns early with the "no Node.js
	...
	err := runNode("npm", []string{"run", "production"}, false)
	if err == nil {
