...
func FPMConfig(version, errorLog string) (string, error) {
	...
	fmt.Fprintf(&b, "; Generated by lerd for native PHP %s. Edits are overwritten.\n", version)
...
func EnsureInstalled(version, binary string) error {
	...
	if err != nil {
		return fmt.Errorf("native PHP %s is not installed (%s)", version, binary)
	}
	if info.IsDir() || info.Mode()&0111 == 0 {
		return fmt.Errorf("native PHP %s at %s is not an executable", version, binary)
	}
	return nil
}
